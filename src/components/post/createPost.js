...
function CreatePost() {
    ...
    const handlecreatePost = (e) => {
        ...
        console.log('here we select from store', state)
    }
            ...
            <div className="p-6 max-w-lg mx-auto bg-gray-600 rounded-xl shadow-lg flex">
                <div className='flex space-x-12'>
                    <div className="shrink-">
                        <span className="h-12 w-12 rounded-full bg-gray-dark p-3">{`${state?.settings?.currentUser?.firstName[0]}${state?.settings?.currentUser?.lastName[0]}`}</span>
                    </div>
